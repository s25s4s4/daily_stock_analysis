...
- [文档] Issue #777 Web UI 语言切换只涉及独立 `dsa.uiLanguage` 持久化与界面文案国际化，不改写 `provider/model/base_url` 运行时配置保存与清理语义；回归依据见 `apps/dsa-web/src/contexts/__tests__/UiLanguageContext.test.tsx`（Storage 读写降级）与 `apps/dsa-web/src/hooks/__tests__/useSystemConfig.test.tsx`（非运行时配置保存保持 `LITELLM_MODEL`、`OPENAI_BASE_URL`、`OPENAI_VISION_MODEL`，不会触发 provider/model/base_url 路由迁移）。
...
- [测试] 本轮 Web 方向验证执行：`cd apps/dsa-web && npm run lint`、`cd apps/dsa-web && npm run build`、`cd apps/dsa-web && npm run test -- src/App.test.tsx src/contexts/__tests__/UiLanguageContext.test.tsx src/hooks/__tests__/useSystemConfig.test.tsx`（均通过）；`cd apps/dsa-web && npm run test:smoke` 在当前沙箱因 `EPERM` 无法监听前端端口失败，需在可达环境补跑并上传截图。
